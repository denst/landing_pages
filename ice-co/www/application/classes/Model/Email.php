...
//            eregi($search.'([^&]*)', $phrase.'&', $phrase2);
    ...
    public function send_email($post, $referer)
    {
        ...
        if(! $this->send_admin_email($post, $referer))
                $is_error = true;
        if(! $this->send_user_email($post))
                $is_error = true;
    ...
    public function send_admin_email($post, $referer)
    {
    ...
    public function send_user_email($post)
    {
        $subject = 'Вы подписались на '.'http://'.$_SERVER['HTTP_HOST'];
        $view = View::factory('email/user')
            ->set('data', $post);
        
        if(Model::factory('Email')->send($post['email'], $subject, $view->render()))
            return true;
        else
            return false; 
